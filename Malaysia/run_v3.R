...
for (i in 1:length(COMPANY_NAMES)){
  ...
  if(file.exists(company_results_path[i])){
    ...
    if (!"bank_name" %in% colnames(company_results)) {
      company_results$bank_name <- "-"
    }
    
    company_results <- cbind(insurer = names(COMPANY_NAMES[i]), company_results)
...
dummy_compilation$product_name[dummy_compilation$product_name == "Qaseh"] <- "BSN Qaseh"
dummy_compilation$product_name[dummy_compilation$product_name == "Qaseh Bakti Plus"] <- "BSN Qaseh Bakti Plus"
