...
complete_url <- function(url){paste0("https://www.generali.com.my", url)}
...
results <- results %>%
  mutate(insurance_type = case_when(
    product_type == "Health insurance" ~ "TBD", # There's a way to automate this by reading "The benefit(s) payable under eligible product is protected by PIDM up to limits. Please refer to PIDM’s ... contact Generali Life Insurance Malaysia Berhad", but short of time so doing it manually for now
    product_type == "Motor & home" ~ "General Insurance",
    product_type == "SAVINGS & LIFE PROTECTION" ~ "Life Insurance",
    product_type == "Travel & Personal Accident" ~ "General Insurance",
    TRUE ~ product_type
  ))
...
results <- results %>%
  mutate(product_type = case_when(
    ...
    product_type == "Motor & home" ~ "Motor/Home",
    product_type == "SAVINGS & LIFE PROTECTION" ~ "Life",
    product_type == "Travel & Personal Accident" ~ "Travel",
    TRUE ~ product_type
  ))
